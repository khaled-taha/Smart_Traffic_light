...
typedef enum {
	GREEN, YELLOW, RED, MAX_SIGNAL,
} Signal;

typedef enum {
	ROAD_SN, ROAD_NS, ROAD_EW, ROAD_WE, MAX_ROAD,
} Road;
...
void handle_NS_SN(void) {
	if (roundi % 2 == 1) {
		State nextState = STATE_NS_NE;
	} else {
		State nextState = STATE_SN_SW;
	}
	// update traffic light states
	// GREEN
	...
	State nextState = check_ambulance_or_density(GREEN_DURATION);// -> delay | state
	// Yellow
...
void handle_EW_WE(void) {
	if (roundx % 2 == 1) {
		State nextState = STATE_EW_ES;
	} else {
		State nextState = STATE_WE_WN;
	}
	// update traffic light states
	// GREEN
	...
	// handle ambulance presence and road density
	State nextState = check_ambulance_or_density(YELLOW_DURATION);// -> delay | state
	// Red
	traffic_lights[LIGHTS_EW] = RED;
	traffic_lights[LIGHTS_WE] = RED;
	// handle ambulance presence and road density
	State nextState = check_ambulance_or_density(RED_DURATION);	// -> delay | state
